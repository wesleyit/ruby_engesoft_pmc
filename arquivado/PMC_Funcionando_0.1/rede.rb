	...
	def treina(padrao_de_treino)
		...
		loop do
			...
			padrao_de_treino.each do |p|
				...
				puts "X:#{p[0]}, d:#{@resultados_desejados}, Y:#{@resultados_obtidos}, Er:#{@erros_locais}, ErQ:#{@erro_quadratico}"
			...
			puts "EMQ = #{@erro_medio_quadratico}"
			@soma_erros_quadraticos = []
			@epocas += 1
			break if @epocas > $limite_de_epocas
			break if @erro_medio_quadratico < $erro_aceitavel
		end
		lista_pesos
	end
end
